...
- **目标 60 帧**。已经实测验证：接收端**稳态 1280×720 / 60 fps**。
  注意第一两秒会略糊 —— 那是 WebRTC 的带宽估计还在爬升（编码器从低码率起步），
  不是设置问题，几秒后会自己恢复到全分辨率。
- **带宽不够时优先保分辨率，而不是保帧率。** 共享屏幕主要是看内容
  （文档、网页、代码），糊掉就没意义了。这个取舍可以改：
  `config.json` 里 `rtc.degradationPreference` 设为 `maintain-framerate` 就会反过来。
- 帧率与码率上限也能调：`rtc.frameRate`（默认 60）、`rtc.maxBitrateMbps`（默认 8）。
